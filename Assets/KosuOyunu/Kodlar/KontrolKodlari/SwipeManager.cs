...
public class SwipeManager : MonoBehaviour
{
    ...
    [Tooltip("Tıklamaların kullanıcı arayüzünden geçmesini önlemek istiyor musunuz?")]
    [SerializeField] private bool preventClicksThroughUI = true;

    [Tooltip("Bir dokunuşu tanımak için maksimum süre")]
    [SerializeField] [Range(0.1f, 1f)] private float maxtapTime = 0.5f;

    [Tooltip("Gerçek bir kaydırma olarak tanımak için minimum kaydırma uzunluğu")]
    [SerializeField] [Range(50f, 200f)] private float minSwipeLength = 100f;

    [Tooltip("Ardışık dokunuşların çift veya üçlü dokunma olarak kabul edileceği zaman")]
    [SerializeField] [Range(0.1f, 1f)] private float timeBetweenTaps = 0.5f;

    [Tooltip("Bu, bir kez çağrıldıktan sonra belirli saniyeler için dokunmmaları geçersiz kılar. Kesintisiz ardışık dokunuşlar istiyorsanız bunu sıfıra ayarlayın")]
    [SerializeField] [Range(0f, 5f)] private float tapNullifyTimeAfterTrigger = 1f;

    [Tooltip("Bu, bir kez çağrıldıktan sonra belirli saniyeler için çift dokunmayı geçersiz kılar. Kesintisiz ardışık dokunuşlar istiyorsanız bunu sıfıra ayarlayın")]
